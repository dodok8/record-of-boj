...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let min_tree: StaticArq<AssignMin> = StaticArq::new(&data);
    let max_tree: StaticArq<AssignMax> = StaticArq::new(&data);
    ...
    for _ in 0..q {
        ...
        writeln!(
            output,
            "{}",
            max_tree.query(start, end) - min_tree.query(start, end)
        )
        .unwrap();
    }
