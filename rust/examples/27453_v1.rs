...
fn get_connected_points(
    ...
    max_w: i32,
    map: &Vec<Vec<i32>>,
    parents: &mut Vec<Vec<Vec<bool>>>,
) -> Vec<(usize, usize)> {
    let mut connected_points: Vec<(usize, usize)> = Vec::new();
    let deltas = [(1, 0), (-1, 0), (0, 1), (0, -1)];
    for (idx, (dh, dw)) in deltas.iter().enumerate() {
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut parents: Vec<Vec<Vec<bool>>> = vec![vec![vec![false; width]; height]; 4];
